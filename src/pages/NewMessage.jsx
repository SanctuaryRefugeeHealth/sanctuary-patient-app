...
class NewMessage extends React.Component {
  ...
  displayMessage() {
    ...
    let messageBody = "";
    messageBody = templates.find(
      (tpl) => tpl.templateId === message.templateId
    )[message.languageId];
    ...
    messageBody = messageBody.replace(
      "${clinic}",
      appointment.clinicName + ", " + appointment.clinicAddress
    );
    return messageBody;
  }
      ...
      <div>
        <form onSubmit={this.createNewMessage} style={{ padding: "1.5rem" }}>
          ...
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            >
              {templates.map((tpl) => (
                <MenuItem value={tpl.templateId} key={tpl.templateId}>
                  {tpl.templateName}
                </MenuItem>
              ))}
            </Select>
