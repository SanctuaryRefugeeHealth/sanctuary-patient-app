...
const errorHandle = (defaultHandle) => (code, message) =>
  (handlers[code] || defaultHandle)(code, message); // currying
const handle = errorHandle((code, message) =>
  Promise.reject(message || `Request error (${code})`)
...
function get(link) {
  return axios.get(`${API_URL}${link}`, header()).then(
...
function post(link, data) {
  return axios.post(`${API_URL}${link}`, data, header()).then(
    (response) => response.data,
    (rejected) =>
      handle(rejected.response.status, rejected.response.data.message)
  );
}

function put(link, data) {
  return axios.put(`${API_URL}${link}`, data, header()).then(
    (response) => response.data,
    (rejected) => handle(rejected.response.status)
  );
}
...
function patch(link, data) {
  return axios.patch(`${API_URL}${link}`, data, header()).then(
    (response) => response.data,
    (rejected) => handle(rejected.response.status)
  );
}
...
function remove(link, data) {
  return axios.delete(`${API_URL}${link}`, data, header()).then(
    (response) => response.data,
    (rejected) => handle(rejected.response.status)
  );
}
...
function login(data) {
  ...
  return axios.post(`${API_URL}/auth`, { email, password: data.password }).then(
    (response) => {
